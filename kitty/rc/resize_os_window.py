...
class ResizeOSWindow(RemoteCommand):
    ...
    action/choices.resize.toggle-fullscreen.toggle-maximized.toggle-always-on-top.toggle-visibility.hide.show.os-panel: The action to perform
...
--action
default=resize
choices=resize,toggle-fullscreen,toggle-maximized,toggle-always-on-top,toggle-visibility,hide,show,os-panel
    ...
    def response_from_kitty(self, boss: Boss, window: Window | None, payload_get: PayloadGetType) -> ResponseType:
        ...
        if windows:
            ...
            for os_window_id in {w.os_window_id for w in windows if w}:
                ...
                elif ac == 'toggle-always-on-top':
                    from kitty.fast_data_types import toggle_always_on_top
                    if toggle_always_on_top(os_window_id) is None:
                        raise RemoteControlErrorWithoutTraceback(
                            f'OS Window {os_window_id}: toggle-always-on-top is not supported '
